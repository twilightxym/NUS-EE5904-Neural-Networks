...
% set of each eopch during training.
    ...
    for i = 1 : epochs
        ...
        fprintf('Acc_train = %.2f, Acc_test = %.2f\n', accu_train(i)*100, accu_val(i)*100)
    end
    % Visualization
    fig = figure;
    plot(1:epochs, accu_train*100, 'bo-', 'LineWidth', 1.5); hold on;
    plot(1:epochs, accu_val*100, 'rs--', 'LineWidth', 1.5);
    xlabel('epochs');
    ylabel('Accuracy(%)');
    legend('Training', 'Test');
    title('Performance');
    grid on;
    saveas(fig,'SeqMode_traingdx_AccuCurves_traingdx_r0.25.png');
end